...
## 1.6.0 (mac) — Portage macOS : mêmes fonctionnalités, moteur JXA/WebKit.

- Moteurs mac : **pas d'ImageMagick** — `sips` (livré avec macOS) remplace
  GDI+ *et* ImageMagick pour png/jpg/bmp/gif/tiff/ico et lit nativement les
  HEIC/HEIF ; les entrées WebP/AVIF et la cible WebP passent par FFmpeg.
  Les écrivains maison ICO 256 et PDF (JPEG `DCTDecode`) sont portés tels
  quels. FFmpeg, ffprobe et Pandoc s'installent via `scripts/deps.sh` du
  hub (builds mac officiels, dossier `dependencies` partagé).
- Amélioration mac : les fichiers s'ajoutent aussi par glisser-déposer
  directement dans la fenêtre (sur Windows, seul le dépôt sur l'exe —
  arguments de ligne de commande — était accepté).
